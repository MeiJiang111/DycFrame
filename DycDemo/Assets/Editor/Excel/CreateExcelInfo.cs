...
public class CreateExcelInfo
{
    ...
    private static string ExcelFilePath = EditorPath.ExcelFilePath;                 //excel文件存放路径
    private static string BinaryFilePath = EditorPath.BinaryFile_Path;              //生成的二进制文件存放路径
    private static string fieldClassPath = EditorPath.fieldClassPath;               //生成的字段文件存放路径
    private static string dicClassPath = EditorPath.dicClassPath;                   //生成的字典文件存放路径

    //key为表名，value为Excel文件名+表名 用来判断表名是否重复
    private static Dictionary<string, string> sheetRepeatName = new Dictionary<string, string>();

    [MenuItem("懒人工具/CreateExcelInfo", false, 1)]
